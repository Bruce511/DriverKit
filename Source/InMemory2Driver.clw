...
! Note the Conditional Compile Symbols in the Project are set;   DRVLM=>1;DRVDM=>0;DRVMEM2LM=>1;DRVMEM2DM=>0
!------------------------------------------------------------------------------------------------
! To build a new driver based on this file;
! 1. Clone this file to your own CLW name
! 2. Adjust the set of equates in block below
! 3. Your Driver Class Name should be used in place of InMemory2Driver
...
! 6. The InMemory2DriverGroup fields should be set as desired by your driver. Care should be taken with this group not to change any sizes.
! 7. ClaMem2.Exp should cloned to match your DLL Name, and the functions in there named to match your ShortName
! 8. The project should have appropriate "Condiional Compilation Symbols". In this case DRVLM=>1;DRVDM=>0;DRVLITE2LM=>1;DRVLITE2DM=>0;DRVMEM2LM=>1;DRVMEM2DM=>0
!------------------------------------------------------------------------------------------------
...
TDescAddress Equate(00a26978h)

  MAP
    InMemory2DriverPipe(Long pOpCode, long pClaFCB, long pVarList),long,name(LongName)
    InMemory2DriverPipeView(Long pOpCode, Long pClaVCB, long pVarList),long
    InMemory2DriverSetObject(Long pOpCode, Long pClaFCB, long pVarList),Long
    InMemory2DriverSetObjectView(Long pOpCode, Long pClaVCB, long pVarList),Long
    !
    module('windows')
...
DRIVER_HAS_BIND_FIELD               Equate(YES)
...
DRIVER_HAS_CREATE_DB                Equate(YES)
DRIVER_HAS_DELETE                   Equate(YES)
DRIVER_HAS_DESTROYf                 Equate(YES)
DRIVER_HAS_DISCONNECT               Equate(DRIVER_HAS_SQL)
...
DRIVER_HAS_GET_RANDOM               Equate(YES)
DRIVER_HAS_GETSTATE                 Equate(DRIVER_HAS_STATE)
DRIVER_HAS_INTERSECT                Equate(YES)
DRIVER_HAS_JOIN_TABLE               Equate(YES)
...
DRIVER_HAS_POINTER64                Equate(YES)
...
DRIVER_HAS_PROJECT_FIELD            Equate(YES)
...
DRIVER_HAS_RECORDS64                Equate(YES)
DRIVER_HAS_RECORDSkey               Equate(DRIVER_HAS_KEYS)
DRIVER_HAS_RECORDS_FILTER           Equate(YES)
...
DRIVER_HAS_SEARCH                   Equate(YES)
DRIVER_HAS_SEND                     Equate(YES)
DRIVER_HAS_SET_CUSTOM_TYPE          Equate(YES)
DRIVER_HAS_SET64                    Equate(YES)
...
DRIVER_HAS_SUBSET                   Equate(YES)
DRIVER_HAS_TO_QUEUE                 Equate(YES)
DRIVER_HAS_UNFIXFORMAT              Equate(NO)
DRIVER_HAS_UNLOCK                   Equate(YES)
DRIVER_HAS_UNION                    Equate(YES)
DRIVER_HAS_UNION_ALL                Equate(YES)
DRIVER_HAS_UPSERT                   Equate(YES)
...
NUM_DRIVER_OPS          Equate(|
                        ...
                        + DRIVER_HAS_COPY                    |
                        + DRIVER_HAS_CREATE                  |
                        + DRIVER_HAS_CREATE_DB               |
                        + DRIVER_HAS_DELETE                  |
                        + DRIVER_HAS_DESTROYf                |
                        + DRIVER_HAS_DISCONNECT              |
                        ...
                        + DRIVER_HAS_GET_RANDOM              |
                        + DRIVER_HAS_GETSTATE                |
                        + DRIVER_HAS_INTERSECT               |
                        + DRIVER_HAS_JOIN_TABLE              |
...
TypeDescriptor  String('' |
                 ...
                 & chr(Opcode:GETRANDOM)       |
                 & chr(Opcode:GETSTATE)        |
                 & chr(Opcode:INTERSECT)       |
                 & chr(Opcode:JOINTABLE)       |
...
InMemory2DriverGroup  Group,name(LongName & '$DrvReg')
...
InMemory2DriverOnLoadDll  Class
Construct                   Procedure()
                          End
...
Result        Long
 ...
 !dbg = '[' & ShortName & '] Driver Pipe: [' & pOpCode & '] ' & ' pClaFCB=' & pClaFCB & ' pVarList = ' & pVarList & ' rblock=' & ClaFCB.rblock ; ods(dbg)
  ...
  of Opcode:WHO_ARE_YOU
    ...
    Return Address(InMemory2DriverPipe)  ! now returning an address to the pipe function
  of Opcode:QUERY_VIEW
    ! doing this special case here, because the address is in scope here, but not in scope in the object.
    Return Address(InMemory2DriverPipeView)
  of Opcode:DESTROYf
...
ViewObject  &DriverViewInMemory2Class,auto
  code
?  !dbg = '[' & ShortName & '] Driver View Pipe: [' & pOpCode & '] ' & ' pClaVCB=' & pClaVCB & ' pVarList = ' & pVarList ; ods(dbg)
  ViewObject &= (InMemory2DriverSetObjectView(pOpCode,pClaVCB,pVarList))
...
InMemory2DriverSetObject  Procedure(Long pOpCode, Long pClaFCB, long pVarList)
ClaFCB        &Cla_FCBBLK,auto
DriverObject  &DriverFileInMemory2Class,auto
f             &File,auto
  Code
  ClaFCB &= (pClaFCB)
  If ClaFCB.rblock = 0
    DriverObject &= new DriverFileInMemory2Class
...
! the program can replace this object using file{prop:object} = whatever
! the program object must be derived from the object type below.
InMemory2DriverSetObjectView  Procedure(Long pOpCode, Long pClaVCB, long pVarList)
ClaVCB        &Cla_VCBBLK,auto
ViewObject    &DriverViewInMemory2Class,auto  ! In this example the root DriverClass is used. In a real driver the actual Driver Class will be used.
v             &View ,auto
  Code
  ClaVCB &= (pClaVCB)
  If ClaVCB.rblock = 0
    ViewObject &= new DriverViewInMemory2Class
    v &= (pClaVCB)
    ViewObject.Init(v)
